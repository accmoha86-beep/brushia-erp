...
function NotificationCenter() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      const stockRes = await fetch('/api/v1/inventory/stock?limit=500', { headers });
      if (stockRes.ok) {
...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 z-50 flex w-64 flex-col bg-gray-900 transition-transform duration-200 lg:static lg:!translate-x-0',
